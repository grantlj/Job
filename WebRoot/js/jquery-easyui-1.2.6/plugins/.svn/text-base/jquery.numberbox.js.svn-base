			...
			.extend( {}, $.fn.validatebox.defaults,
					{
						...
						formatter : function(_20) {
							...
							if (_21.groupSeparator) {
								...
								while (p.test(s1)) {
									s1 = s1.replace(p, "$1"
											+ _21.groupSeparator + "$2");
								}
							}
							if (s2) {
								return _21.prefix + s1 + _21.decimalSeparator
										+ s2 + _21.suffix;
							} else {
						...
						parser : function(s) {
							...
							if (_23.groupSeparator) {
								s = s.replace(new RegExp("\\"
										+ _23.groupSeparator, "g"), "");
							}
							if (_23.decimalSeparator) {
								s = s.replace(new RegExp("\\"
										+ _23.decimalSeparator, "g"), ".");
							}
							if (_23.prefix) {
								s = s.replace(new RegExp("\\"
										+ $.trim(_23.prefix), "g"), "");
							...
							if (isNaN(val)) {
								...
								if (typeof (_23.min) == "number"
										&& val < _23.min) {
									val = _23.min.toFixed(_23.precision);
								} else {
									if (typeof (_23.max) == "number"
											&& val > _23.max) {
